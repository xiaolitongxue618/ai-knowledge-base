...
class RAGChain:
    ...
    def ask(
        ...
        try:
            ...
            # 4. 构建提示词
            prompt = f"""你是一个专业、友好的AI学习助手。请根据以下参考信息准确回答用户的问题。
...
【核心原则】
1. **准确性第一**：只使用参考信息中的内容，绝对不要编造或添加外部知识
2. **友好表达**：用自然、清晰的语气解释，可以适当使用鼓励的话语

【回答要求】
- 如果参考信息有明确答案：直接、准确地解释，可以引用关键信息
- 如果参考信息不足以回答：坦诚说明"根据现有资料，我无法完整回答这个问题"
- 如果参考信息完全不相关：直接说明"资料中没有涉及这个内容"
- 避免为了"友好"而编造不存在的内容
